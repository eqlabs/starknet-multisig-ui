...
const MultisigList = () => {
  ...
  return (
    <InnerContainer css={{marginTop: "$6", gap: "0"}}>
      ...
      {multisigs?.map((contract, _index) => (
        <Multisig key={`contractList-${contract.address}`} css={{margin: "0", padding: "$4 0", borderBottom: "1px $borderColor solid"}}>
            <LinkWrapper>
              <Address>
                <InvisibleButton onClick={() => deleteMultisigFromCache(contract.address)}><FiX size="17px" /></InvisibleButton>
                <InvisibleButton onClick={() => navigator.clipboard.writeText(contract.address)}><FiCopy size="17px" /></InvisibleButton>
                <Link href={`/multisig/${contract.address}`} passHref>{truncateAddress(contract.address, 14)}</Link>
              </Address>
              <div style={{ display: "flex", flexDirection: "row", gap: "1rem", alignItems: "center" }}>
                ...
                <Link href={`/multisig/${contract.address}`} passHref><RightArrow css={{flexShrink: "0", stroke: "$text", height: "3rem", width: "3rem"}}/></Link>
              </div>
            </LinkWrapper>
        </Multisig>
      ))}
