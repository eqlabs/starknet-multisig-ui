    ...
    <FrontPageWrapper>
      ...
      {!pendingWallet ? (<>
        <div style={{ display: "flex", flexDirection: "row", gap: "1rem", justifyContent: "space-between" }}>
          <Paragraph css={{ fontSize: "$lg", margin: "0", color: "$textMuted", width: "$xl", alignItems: "flex-start", justifyContent: "flex-start" }}>
          ...
          {getInstalledInjectedConnectors().map(wallet => (
            <Button size="md" key={wallet.id()} fullWidth onClick={() => connectCallback(wallet)}>
